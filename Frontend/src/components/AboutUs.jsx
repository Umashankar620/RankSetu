...
const STORY_STEPS = [
  ...
  },
  {
    ...
    body: "Instead of accepting the chaos, I decided to build what didn't exist — a single, reliable platform that aggregates MCC cutoffs, AYUSH data, college databases, and counselling guidance in one place.",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    body: "RankSetu launched with real MCC Opening & Closing Rank data, a Choice Optimizer, and an Upgrade Checker — making counselling insights more accessible and easier to understand for NEET aspirants.",
  ...
  },
  {
    ...
    body: "Today RankSetu covers 991 colleges, 21 AIIMS, state quotas across 28 states, AYUSH counselling, a rank predictor, and a live timeline — 100% free. Every feature exists because a student genuinely needed it.",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function DataSourcesSection({ dm }) {
  const SOURCES = [
    { name: "MCC", full: "All India Quota — MBBS/BDS Counselling" },
    { name: "AACC", full: "AIQ BSc Nursing & AYUSH Counselling" },
    { name: "NMC", full: "National Medical Commission" },
    { name: "State NEET Cells", full: "All 28 States — UG Medical Counselling" },
    { name: "State AYUSH Cells", full: "All States — BAMS/BHMS/BUMS Counselling" },
    { name: "UP NEET / UP AYUSH", full: "Uttar Pradesh State Counselling" },
  ];
  return (
    <div className="mb-14">
      <SectionTitle
        eyebrow="Where Our Data Comes From"
        title="Official"
        highlight="Data Sources"
        subtitle="We do not create or alter government data. We collect it from official publications — MCC, AACC, and every state's NEET and AYUSH counselling authority — and present it in one clear, structured format."
        dm={dm}
      />
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
        {SOURCES.map((s) => (
          <div key={s.name} className={`p-5 rounded-xl border text-center
            ${dm ? 'bg-slate-800/40 border-slate-700' : 'bg-white border-slate-200 shadow-sm'}`}>
            <p className={`font-bold text-lg mb-1 ${dm ? 'text-white' : 'text-[#1A3C6E]'}`}>{s.name}</p>
            <p className={`text-xs leading-snug ${dm ? 'text-slate-400' : 'text-slate-500'}`}>{s.full}</p>
          </div>
        ))}
      </div>
      <p className={`text-xs leading-relaxed max-w-2xl ${dm ? 'text-slate-500' : 'text-slate-400'}`}>
        RankSetu is not affiliated with, endorsed by, or officially connected to any of the above authorities. All names are used solely to indicate the origin of publicly available counselling data.
      </p>
    </div>
  );
}

// ─── Coverage Section — "All-in-One" positioning ──────────────────────────────
function CoverageSection({ dm }) {
  const COVERAGE = [
    { tag: "MCC · All India Quota & Nursing", desc: "MBBS & BDS & BSc Nursing — 15% AIQ seats across India" },
  ...
  return (
    <div className={`p-8 md:p-10 rounded-xl border shadow-sm mb-14
      ${dm ? 'bg-[#0D1829] border-slate-700' : 'bg-white border-slate-200'}`}>
      <div className="mb-3"><Badge dm={dm}>One Platform. Everything Covered.</Badge></div>
      <h2 className={`text-3xl sm:text-4xl font-bold tracking-tight leading-tight mb-3
        ${dm ? 'text-white' : 'text-[#1A3C6E]'}`}>
        Not Just MCC. Not Just AYUSH. <span className="text-[#2563EB]">Everything, in One Place.</span>
      </h2>
      <p className={`text-base leading-relaxed max-w-2xl mb-8 ${dm ? 'text-slate-300' : 'text-slate-600'}`}>
        Most sources only show you one piece — an MCC list here, a state AYUSH PDF there. RankSetu is the
        only platform aspirants need: All India Quota (MCC) &amp; Nursing counselling , AACC AYUSH , and
        every state's NEET UG and AYUSH counselling — including UP NEET and UP AYUSH — all searchable
        from a single, simple dashboard.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {COVERAGE.map((c) => (
          <div key={c.tag} className={`flex items-start gap-3 p-4 rounded-lg border
            ${dm ? 'bg-slate-800/40 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
            <span className={`mt-1 w-2 h-2 rounded-full shrink-0 bg-[#2563EB]`} />
            <div>
              <p className={`font-bold text-sm mb-0.5 ${dm ? 'text-white' : 'text-[#1A3C6E]'}`}>{c.tag}</p>
              <p className={`text-xs leading-relaxed ${dm ? 'text-slate-400' : 'text-slate-500'}`}>{c.desc}</p>
            </div>
          </div>
        ))}
      </div>
...
// ─── FAQ Section ────────────────────────────────────────────────────────────────
function FAQItem({ q, a, dm, isOpen, onClick }) {
  return (
    <div className={`rounded-xl border overflow-hidden ${dm ? 'bg-slate-800/40 border-slate-700' : 'bg-white border-slate-200 shadow-sm'}`}>
      <button
        onClick={onClick}
        className="w-full flex items-center justify-between gap-4 p-5 text-left"
      >
        <span className={`font-bold text-sm sm:text-base ${dm ? 'text-white' : 'text-[#1A3C6E]'}`}>{q}</span>
        <span className={`shrink-0 w-7 h-7 rounded-full border flex items-center justify-center transition-transform
          ${isOpen ? 'rotate-45' : ''}
          ${dm ? 'border-slate-600 text-slate-300' : 'border-slate-300 text-slate-500'}`}>
          +
        </span>
      </button>
      <div className={`px-5 transition-all duration-300 ${isOpen ? 'max-h-40 pb-5 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <p className={`text-sm leading-relaxed ${dm ? 'text-slate-400' : 'text-slate-600'}`}>{a}</p>
      </div>
    </div>
  );
}
...
function FAQSection({ dm }) {
  ...
  const FAQS = [
    { q: "Is RankSetu an official MCC or government website?", a: "No. RankSetu is an independent, student-built platform. We are not affiliated with MCC, NMC, DGHS, or any State Counselling Authority. We compile and present publicly available official data to make it easier to understand." },
    { q: "Is RankSetu really free to use?", a: "Yes, completely. Every feature — cutoff data, rank predictor, choice optimizer, and counselling guides — is free, with no hidden charges, subscriptions, or paywalls." },
    { q: "Where does the cutoff and rank data come from?", a: "All data is sourced from official publications released by MCC, NMC, DGHS, and respective State Counselling Authorities, then structured for easy reading." },
    // { q: "Should I make my final decision based only on RankSetu?", a: "RankSetu is meant to support your decision-making, not replace official verification. Always cross-check your final choices with the official counselling website before submitting preferences." },
  ];
  return (
    <div className="mb-14">
      <SectionTitle
        ...
        highlight="Questions"
        subtitle="A few things aspirants and parents often ask us before trusting the platform."
        dm={dm}
      />
      <div className="space-y-3">
        {FAQS.map((f, i) => (
          <FAQItem key={i} {...f} dm={dm} isOpen={open === i} onClick={() => setOpen(open === i ? -1 : i)} />
        ))}
      </div>
    </div>
  );
...
// function ContactSection({ dm }) {
//   return (
//     <div className={`p-8 md:p-10 rounded-xl border shadow-sm mb-14
//       ${dm ? 'bg-slate-800/30 border-slate-700' : 'bg-white border-slate-200'}`}>
//       <div className="mb-6"><Badge dm={dm}>Contact & Grievance</Badge></div>
//       <h2 className={`text-2xl sm:text-3xl font-bold tracking-tight mb-4 ${dm ? 'text-white' : 'text-[#1A3C6E]'}`}>
//         Questions, Corrections, or Concerns?
//       </h2>
//       <p className={`text-sm leading-relaxed max-w-2xl mb-6 ${dm ? 'text-slate-300' : 'text-slate-600'}`}>
//         Found an error in the data, or have feedback about the platform? We take accuracy seriously and respond to every genuine query.
//       </p>
//       {/* <div className="flex flex-wrap gap-4">
//         <a href="mailto:support@ranksetu.com" className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold border transition-colors
//           ${dm ? 'bg-[#1A3C6E]/15 border-[#1A3C6E]/30 text-[#93B4DC] hover:bg-[#1A3C6E]/25' : 'bg-[#1A3C6E]/5 border-[#1A3C6E]/20 text-[#1A3C6E] hover:bg-[#1A3C6E]/10'}`}>
//           support@ranksetu.com
//         </a>
//         <a href="mailto:grievance@ranksetu.com" className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold border transition-colors
//           ${dm ? 'bg-slate-700/50 border-slate-600 text-slate-200 hover:bg-slate-700' : 'bg-slate-100 border-slate-200 text-slate-600 hover:bg-slate-200'}`}>
//           grievance@ranksetu.com — Grievance Officer
//         </a>
//       </div> */}
//     </div>
//   );
...
export default function AboutUs({ darkMode: dm = false }) {
  ...
  const STATS = [
    { value: "991",  label: "Medical Colleges", suffix: "+" },
  ...
  return (
    <div className={`min-h-screen ${dm ? 'bg-[#0A0F19]' : 'bg-[#F9FAFC]'}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        ...
        {/* ── 1B. COVERAGE — All-in-One positioning ────────────────────── */}
        <CoverageSection dm={dm} />

        {/* ── 2. STATS ─────────────────────────────────────────────────── */}
        <div ref={statsRef} className="flex flex-wrap gap-4 mb-3">
          {STATS.map((s, i) => (
            <StatCard key={s.label} value={s.value} label={s.label} dm={dm} active={statsVis} delay={i * 100} />
          ))}
        </div>
        <p className={`text-xs mb-14 ${dm ? 'text-slate-500' : 'text-slate-400'}`}>
          Data last verified: {LAST_UPDATED}
        </p>
        ...
        {/* ── 8. FOUNDER MESSAGE ───────────────────────────────────────── */}
        <div className={`rounded-xl overflow-hidden border shadow-sm mb-14 ${dm ? 'border-slate-700' : 'border-slate-200'}`}>
          {/* Top accent */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-10 lg:gap-16">
              ...
              <div>
                ...
                <div className="flex items-center justify-center gap-3 mt-4">
                  <a href="https://www.linkedin.com/in/umashankar-kushwaha-2a06a02ba" target="_blank" rel="noopener noreferrer"
                    aria-label="Founder's LinkedIn"
                    className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-colors
                      ${dm ? 'bg-slate-800/50 border-slate-700 text-slate-300 hover:text-[#60A5FA] hover:border-[#1A3C6E]/50' : 'bg-slate-50 border-slate-200 text-slate-500 hover:text-[#1A3C6E] hover:border-[#1A3C6E]/30'}`}>
                  ...
                  <a href="https://www.instagram.com/umash.ankar8?igsh=c2I5aGRwZzcydGI2" target="_blank" rel="noopener noreferrer"
                    aria-label="Founder's Instagram"
                    className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-colors
                      ${dm ? 'bg-slate-800/50 border-slate-700 text-slate-300 hover:text-[#60A5FA] hover:border-[#1A3C6E]/50' : 'bg-slate-50 border-slate-200 text-slate-500 hover:text-[#1A3C6E] hover:border-[#1A3C6E]/30'}`}>
                  ...
                  {/* <a href="https://x.com/YOUR-X-HANDLE" target="_blank" rel="noopener noreferrer"
                    aria-label="Founder's X (Twitter)"
                    className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-colors
                      ${dm ? 'bg-slate-800/50 border-slate-700 text-slate-300 hover:text-[#60A5FA] hover:border-[#1A3C6E]/50' : 'bg-slate-50 border-slate-200 text-slate-500 hover:text-[#1A3C6E] hover:border-[#1A3C6E]/30'}`}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M18.24 2.25h3.31l-7.23 8.26 8.5 11.24H16.2l-5.21-6.82-5.96 6.82H1.72l7.73-8.84L1.28 2.25H8.1l4.71 6.23zm-1.16 17.52h1.83L7.02 4.13H5.06z"/></svg>
                  </a> */}
                </div>
              </div>
        ...
        {/* ── 9. DATA SOURCES ──────────────────────────────────────────── */}
        <DataSourcesSection dm={dm} />

        {/* ── 10. FAQ ──────────────────────────────────────────────────── */}
        <FAQSection dm={dm} />

        {/* ── 11. CONTACT / GRIEVANCE ──────────────────────────────────── */}
        {/* <ContactSection dm={dm} /> */}

        {/* ── 12. DISCLAIMER ───────────────────────────────────────────── */}
        <div className={`p-5 rounded-lg border text-xs leading-relaxed
          ${dm ? 'bg-slate-800/40 border-slate-700 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
          <strong className={dm ? 'text-slate-300' : 'text-slate-600'}>Disclaimer: </strong>
          RankSetu is an independent, student-run platform and is not affiliated with, endorsed by,
          or officially connected to MCC, NMC, DGHS, or any State Counselling Authority. All data displayed
          is compiled from publicly available official sources for informational purposes only. Aspirants
          are strongly advised to cross-verify all information with the respective official counselling
          websites before making any final decisions.
        </div>
